...
function WaveIndicator({ reduced }: { reduced: boolean }) {
  if (reduced) {
    return <div className="flex gap-0.5 items-end h-4">{[...Array(5)].map((_, i) => (
      <div key={i} className="w-0.5 bg-amber-500" style={{ height: '50%' }} />
    ))}</div>;
  }
  ...
  return (
    <div className="flex gap-0.5 items-end h-4">
      {[...Array(5)].map((_, i) => (
        <motion.div
          key={i}
          className="w-0.5 bg-gradient-to-t from-amber-500 to-orange-500 rounded-full"
...
function BrainIndicator({ phase, reduced }: { phase: StreamPhase; reduced: boolean }) {
  return (
    ...
    >
      <Brain className={cn(
        'h-5 w-5',
        phase === 'streaming' ? 'text-orange-500' : 'text-amber-600'
      )} />
    </motion.div>
  );
}
