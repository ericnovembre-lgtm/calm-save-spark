...
export function MobileOnboardingFlow({ onComplete, onSkip }: { onComplete: () => void; onSkip: () => void }) {
  ...
  const handleNext = async () => {
    ...
    if (isLastStep) { confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } }); triggerHaptic('success'); onComplete(); }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-background flex flex-col">
      <div className="flex items-center justify-between p-4">
        ...
        <Button variant="ghost" size="sm" onClick={() => { triggerHaptic('light'); onSkip(); }}>Skip <X className="ml-1 h-4 w-4" /></Button>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        <AnimatePresence mode="wait">
          <motion.div key={step.key} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-sm text-center">
            ...
            {step.gesture && <div className="mt-8"><GestureGuide type={step.gesture} onComplete={() => { setGestureSuccess(true); triggerHaptic('success'); }} completed={gestureSuccess} /></div>}
      ...
      <div className="p-6 flex items-center justify-between">
        <Button variant="ghost" onClick={() => { triggerHaptic('light'); setCurrentStep(p => Math.max(0, p - 1)); setGestureSuccess(false); }} disabled={currentStep === 0}><ChevronLeft className="h-4 w-4" />Back</Button>
